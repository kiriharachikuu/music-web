 ...
 * - SSR 默认拉取 GET /api/rankings?ranking=soar（综合-飙升榜）
 * - 交由 RankingsClient 处理 3 个子榜（飙升 / 热歌 / 新歌）切换与播放交互
 */
export const revalidate = 60;

export default async function RankingsPage() {
  // SSR 仅做默认榜单的首屏骨架占位；客户端切 Tab 时走 getRankings(ranking)
  const data = await serverFetch<RankingsResponse>(
    "/rankings?ranking=soar",
    60,
  );
